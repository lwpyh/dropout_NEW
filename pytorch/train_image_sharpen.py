...
############################new#######################
def get_thresh(loader, model, ad_net, th1 = 0.05, th2 = 0.2):
    ...
    for p in iter(loader["source"]):
        ...
        ad_out = ad_net(op_out.view(-1, softmax_output.size(1) * features.size(1))).detach()
        source_range.extend(ad_out.cpu().numpy())
    for q in iter(loader["target"]):
        features, outputs = model(q[0].cuda())
        softmax_output = nn.Softmax(dim=1)(outputs).detach()
        op_out = torch.bmm(softmax_output.unsqueeze(2), features.unsqueeze(1))
        ad_out = ad_net(op_out.view(-1, softmax_output.size(1) * features.size(1))).detach()
        target_range.extend(ad_out.cpu().numpy())  #get all samples' domain output

    for m in source_range + target_range:
        if np.abs(m - 0.5) < 0.05:
            thresh_range1.append(np.abs(1-2*m))
        elif np.abs(m - 0.5) < 0.2:
            thresh_range2.append(np.abs(1-2*m))
        else:
            thresh_range3.append(np.abs(1-2*m))
    thresh1 = 1 / len(thresh_range1) * np.sum(thresh_range1) if len(thresh_range1) else 0
    thresh2 = 1 / len(thresh_range2) * np.sum(thresh_range2) if len(thresh_range2) else 0
    thresh3 = 1 / len(thresh_range3) * np.sum(thresh_range3) if len(thresh_range3) else 0
...
def train(config):
    ...
    for i in range(config["num_iterations"]):
        ...
        if config['method'] == 'CDAN+E':
            ...
            od2 = torch.where(torch.abs(od.view(1, -1) - 0.5) < 0.2, od1, list_t3)  # new
            # print(od,od1,od2)
            transfer_loss1, _ = loss1.CDAN1([features, softmax_out], ad_net1, od2, None, None, random_layer)
            transfer_loss = transfer_loss0 + 2 * transfer_loss1
