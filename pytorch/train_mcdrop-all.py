...
def get_monte_carlo_predictions(input_list,
    ...
    dropout_cal = dropout_predictions - 0.5
    dropout_cal = np.abs(dropout_cal)
    mean = np.mean(dropout_predictions, axis=0)  # shape (n_samples, n_classes)
    dropcal_mean = np.mean(dropout_cal, axis=0)
...
def get_mc_var(dset_loaders,base_network, ad_net,random_layer, src_ones,tar_ones):
    with torch.no_grad():
        ...
        src_variance, src_cal = get_monte_carlo_predictions([src_features, src_softmax_out], 10, ad_net, 1, 498, random_layer)
        tar_variance, tar_cal = get_monte_carlo_predictions([tar_features, tar_softmax_out], 10, ad_net, 1, 2817, random_layer)
        ...
        src_transferbility1 = torch.tensor(src_cal) / torch.max(torch.tensor(src_cal))
        thresh_src1 = get_cls_thresh(src_transferbility1)
        tar_transferbility1 = torch.tensor(tar_cal) / torch.max(torch.tensor(tar_cal))
        thresh_tar1 = get_cls_thresh(tar_transferbility1)
        ...
        src_transferbility = torch.where(torch.tensor(src_transferbility).view(1, -1).cuda() > thresh_src1.cuda(), src_ones,
                                         torch.zeros_like(src_ones))
        tar_transferbility = torch.where(torch.tensor(tar_transferbility).view(1, -1).cuda() > thresh_tar1.cuda(), tar_ones,
                                         torch.zeros_like(tar_ones))
        print(src_transferbility)
        print("ahhh")
        print(tar_transferbility)
        print("max_value", torch.max(torch.tensor(src_cal)), torch.max(torch.tensor(tar_cal)))
        print("thresh", thresh_src1, thresh_tar1)
        src_transferbility1 = torch.where(torch.tensor(src_transferbility1).view(1, -1).cuda() > thresh_src.cuda(),
                                         src_ones,
                                         torch.zeros_like(src_ones))
        tar_transferbility1 = torch.where(torch.tensor(tar_transferbility1).view(1, -1).cuda() > thresh_tar.cuda(),
                                         tar_ones,
                                         torch.zeros_like(tar_ones))
        print(src_transferbility1)
        print("ahhh")
        print(tar_transferbility1)
        src_transferbility_all = src_transferbility * src_transferbility1 + torch.abs(src_transferbility - src_transferbility1)
        tar_transferbility_all = tar_transferbility * tar_transferbility1 + torch.abs(tar_transferbility - tar_transferbility1)
    return src_transferbility_all,tar_transferbility_all
def train(config):
    ## set pre-process
