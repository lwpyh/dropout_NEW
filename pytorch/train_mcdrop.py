...
def train(config):
    ...
    for i in range(config["num_iterations"]):
        ...
        variance = get_monte_carlo_predictions([features, softmax_out], 10, ad_net, 1, 72,random_layer)
        # print(variance)
        thresh = get_cls_thresh(variance)
        od = torch.where(torch.tensor(variance).view(1, -1).cuda() > thresh , ones, zeros)  # new
        # od = torch.FloatTensor(variance).view(1, -1)/ torch.max(torch.FloatTensor(variance))
        od = od.cuda()
        # print(od)
        #####################################################
        if config['method'] == 'CDAN+E':
            ...
            transfer_loss = transfer_loss0 + transfer_loss1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--s_dset_path', type=str, default='../data/office/dslr_31_list.txt',
    ...
    parser.add_argument('--output_dir', type=str, default='d-a-gui1',
                        help="output directory of our model (in ../snapshot directory)")
    parser.add_argument('--lr', type=float, default=0.001, help="learning rate")
    parser.add_argument('--random', type=bool, default=False, help="whether use random projection")
    parser.add_argument('--seed', type=float, default=333, help="interval of seed")
